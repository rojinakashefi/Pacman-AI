...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        "*** YOUR CODE HERE ***"
        if self.getLegalActions(state):
            best_q_values = self.computeValueFromQValues(state)
            actions_max_qValues = []
            for action in self.getLegalActions(state):
                if best_q_values == self.getQValue(state, action):
                    actions_max_qValues.append(action)
            return random.choice(actions_max_qValues)
        return None
